...
- 시료 ID 채번 규칙 (예: `S-NNN`)
- 데이터 모니터링 Tool을 "메인 메뉴 노출 항목"/"숨김 메뉴"/"별도 실행 파일" 중 어느 것으로 둘지
  최종 확정 (Dummy 데이터 생성 Tool의 노출 방식은 6.2에서 확정됨)
...
- **정렬 특성**: 고정 자릿수(6자리) 덕분에 문자열 오름차순 정렬이 곧 발급 순서(=생성 순서)와
  일치한다. 이 특성은 데이터 모니터링 Tool이 생산 큐/접수 큐의 FIFO 순서를 파일 데이터만으로
  재구성할 때 사용된다 ([design_phase_8.md](./design_phase_8.md) 참조).
...
- 메뉴 예시(정확한 번호/문구는 구현 시 자유롭게 조정 가능, [design_phase_8.md - 2](./design_phase_8.md#2-dummy-데이터-생성-tool) 참조):
  ...
  [0] 종료
  ```
- 데이터 모니터링 Tool의 노출 방식은 아직 미정이며(5절 TBD 참조), Dummy 데이터 생성 Tool과
  반드시 같은 방식을 따를 필요는 없다.
