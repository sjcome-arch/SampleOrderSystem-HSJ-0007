...
[design.md](./docs/DESIGN/design.md)를, AI 작업물 검증 절차는 [test.md](./docs/TEST/test.md)를
참조한다. 원본 과제 PDF: `[CRA_AI] Day3_개인과제_반도체시료관리.pdf`.

- 요구사항(무엇을 만들 것인가)이 바뀌면 `REQUIREMENT.md`를, 구현 결정(어떻게 만들 것인가)이 바뀌면
  `docs/DESIGN/design.md`를, 검증 절차가 바뀌면 `docs/TEST/test.md`를 업데이트한다.
...
- **"OOO 커밋해줘"처럼 사용자의 요청 메시지 자체에 "커밋해줘"가 포함되어 있어도, 그 요청 한 번을
  승인으로 간주하지 않는다.** 반드시 (1) 변경 요약과 커밋 메시지 초안을 먼저 보여주는 턴과,
  (2) 사용자가 그 초안을 보고 별도로 확인해주는 턴을 나눈다. 초안을 보여준 것과 같은 턴에서
  바로 `git commit`을 실행하지 않는다.
